...
def test_application_images_pin_immutable_base_images() -> None:
    ...
    assert dockerfile.startswith("FROM python:3.12.14-slim-bookworm@sha256:")
    assert "FROM node:22-bookworm-slim@sha256:" in render_dockerfile
    assert "FROM python:3.12.14-slim-bookworm@sha256:" in (render_dockerfile)
...
def test_database_role_policy_protects_sensitive_tables() -> None:
    ...
    assert "FROM pg_tables" in policy
    assert "REVOKE ALL ON TABLE %I FROM seo_export, seo_scheduler" in policy
    ...
    assert "GRANT UPDATE (id) ON TABLE websites TO seo_integration" in policy
