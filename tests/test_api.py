...
def test_information_architecture_and_legacy_routes_are_served(client: TestClient) -> None:
    ...
    assert 'app.js?v=20260811-4' in page.text
...
def test_guided_website_verification_interface_is_served(client: TestClient) -> None:
    ...
    for element_id in [
        ...
        "first-crawl-preferences",
        "first-crawl-max-urls",
    ]:
        assert f'id="{element_id}"' in page.text
    ...
    for element_id in [
        ...
        "first-crawl-progress-bar",
        "retry-first-crawl",
        "view-first-results",
        "onboarding-measurement-quality",
        "configure-onboarding-measurement",
    ]:
        assert f'id="{element_id}"' in page.text
    ...
    assert "apiResponse" in script.text
    assert "Invalid API key" not in script.text
    assert "resumeWebsiteOnboarding" in script.text
    assert "startFirstOnboardingCrawl" in script.text
    assert 'credentials: "same-origin"' in script.text


def test_main_client_can_be_created_without_website_from_interface(client: TestClient) -> None:
    page = client.get("/ui/assets/index.html")
    assert page.status_code == 200
    assert "Nieuwe hoofdklant" in page.text
    assert "Website optioneel" in page.text
    assert "Hoofdklant aanmaken" in page.text
    assert 'id="onboarding-website-name"' not in page.text
...
def test_operations_status_ignores_stale_website_responses(client: TestClient) -> None:
    page = client.get("/ui/assets/index.html")
    assert page.status_code == 200
    assert 'src="/ui/assets/app.js?v=20260811-4"' in page.text
...
def test_content_analysis_interface_exposes_evidence_and_coverage(client: TestClient) -> None:
    ...
    assert 'href="/ui/assets/external-evidence.css?v=20260808-4"' in page.text
    ...
    assert "function renderQuestionScopes" in script.text
    assert "Controleer AI-dekking" in script.text
    assert "dataforseo" not in script.text.lower()
...
def test_issue_detail_returns_live_element_location(
    ...
    disabled_recheck = client.post(f"/api/v1/issues/{issue_id}/inspection/recheck")
    assert disabled_recheck.status_code == 409
    ...
    assert refreshed["pages"][0]["screenshot_available"] is True
